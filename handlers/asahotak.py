...
def _cleanup_games():
    now = time.time()
    expired = [key for key, game in ASAHOTAK_GAMES.items() if now - game.get("created_at", now) > GAME_TTL]
    for key in expired:
        ASAHOTAK_GAMES.pop(key, None)
...
def _format_question(data: dict) -> str:
    ...
    return (
        ...
        "Reply to this message with the correct answer."
    )

async def asahotak_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    _cleanup_games()
    ok, payload = await neoxr_get("asahotak")
    if not ok:
    ...
    log.info(
        "Asahotak game created | chat_id=%s message_id=%s question=%s answer=%s",
        chat.id,
        sent.message_id,
        question or "-",
    ...
    ASAHOTAK_GAMES[_game_key(chat.id, sent.message_id)] = {
        "answer": answer,
...
async def asahotak_answer_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = update.effective_message
    chat = update.effective_chat
