...
async def _download_replied_media(bot, msg) -> tuple[str, str]:
    ...
    if not target:
        raise RuntimeError("Reply to a photo, image document, or static sticker first.")
    ...
    if target.photo:
        ...
        raise RuntimeError("Reply to a photo, image document, or static sticker.")
    input_path = os.path.join(TMP_DIR, _safe_name(filename))
    await tg_file.download_to_drive(input_path)
    if not os.path.exists(input_path) or os.path.getsize(input_path) <= 0:
        raise RuntimeError("Failed to download the image from Telegram.")
...
def _convert_image_to_jpg(src_path: str) -> str:
    if not shutil.which("ffmpeg"):
        raise RuntimeError("FFmpeg is required to convert the image before upload.")
    out_path = os.path.join(TMP_DIR, f"upscale_{uuid.uuid4().hex}.jpg")
    result = subprocess.run(
        ["ffmpeg", "-y", "-i", src_path, "-frames:v", "1", "-q:v", "2", out_path],
...
async def _upload_to_tmpfiles(path: str) -> str:
    ...
    if not os.path.exists(path):
        raise RuntimeError("Tmpfiles upload file does not exist.")
    size = os.path.getsize(path)
    if size <= 0:
        raise RuntimeError("Tmpfiles upload file is empty.")
    content_type = _guess_content_type(path)
    filename = _safe_name(os.path.basename(path), "image.jpg")
    log.info("Tmpfiles upload start | file=%s size=%s content_type=%s", path, size, content_type)
    session = await _shared_http_session()
    fh = open(path, "rb")
    try:
        form = aiohttp.FormData()
        form.add_field("file", fh, filename=filename, content_type=content_type)
        async with session.post(TMPFILES_UPLOAD_API, data=form, timeout=UPSCALE_TIMEOUT) as resp:
...
async def _call_neoxr_upscale(image_url: str) -> dict:
    ...
    async with session.get(NEOXR_UPSCALE_API, params=params, timeout=UPSCALE_TIMEOUT) as resp:
    ...
    if not data.get("status"):
        raise RuntimeError(data.get("message") or "Upscale failed.")
    result = data.get("data") or {}
    if isinstance(result, str):
        result = {"url": result}
    if not isinstance(result, dict):
        raise RuntimeError("Invalid upscale result.")
    return result

def _pick_result_url(result: dict) -> str:
    for key in ("downloadUrl", "download_url", "url", "image", "result"):
        value = str(result.get(key) or "").strip()
        if value.startswith(("http://", "https://")):
            return value
    raise RuntimeError("Upscale result has no download URL.")
...
async def _download_with_aria2c(url: str, out_path: str):
    ...
    if not os.path.exists(out_path) or os.path.getsize(out_path) <= 0:
        raise RuntimeError("Upscaled file was not downloaded.")
...
async def upscale_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await status.edit_text("<b>Preparing image...</b>", parse_mode="HTML")
        upload_path = _convert_image_to_jpg(input_path)
        converted_path = upload_path
        await status.edit_text("<b>Uploading image...</b>", parse_mode="HTML")
        image_url = await _upload_to_tmpfiles(upload_path)
        await status.edit_text("<b>Processing upscale...</b>", parse_mode="HTML")
        result = await _call_neoxr_upscale(image_url)
        ...
        output_path = os.path.join(TMP_DIR, f"upscaled_{uuid.uuid4().hex}{ext}")
        await status.edit_text("<b>Downloading result...</b>", parse_mode="HTML")
        await _download_with_aria2c(download_url, output_path)
        await status.edit_text("<b>Sending result...</b>", parse_mode="HTML")
        with open(output_path, "rb") as f:
            await msg.reply_photo(
                photo=f,
                caption="<b>Upscale completed</b>",
                parse_mode="HTML",
                reply_to_message_id=msg.reply_to_message.message_id if msg.reply_to_message else msg.message_id,
            )
        await status.delete()
    except Exception as e:
        err = html.escape((str(e) or repr(e)).strip())[:3500]
        try:
            await status.edit_text(f"<b>Upscale failed</b>\n\n<code>{err}</code>", parse_mode="HTML")
        except Exception:
            pass
    finally:
