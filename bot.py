...
# sticker/photo/createpack/addtpack
async def tosticker_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.reply_to_message:
        return await update.message.reply_text("Reply ke photo/sticker yang mau dijadiin sticker.")
    reply = update.message.reply_to_message
    try:
        if reply.sticker:
            f = await context.bot.get_file(reply.sticker.file_id)
            ...
            return await update.message.reply_text("Reply harus ke photo atau sticker.")
    except Exception as e:
        logger.exception("tosticker: failed to get file")
        return await update.message.reply_text(f"Gagal ambil file: {e}")
    webp_img = resize_image_for_sticker(img)
    webp_bio = image_to_webp_bytes(webp_img)
    try:
        webp_bio.seek(0)
        await context.bot.send_sticker(chat_id=update.effective_chat.id, sticker=InputFile(webp_bio, filename="sticker.webp"))
    except Exception:
        webp_bio.seek(0)
        await context.bot.send_document(chat_id=update.effective_chat.id, document=InputFile(webp_bio, filename="sticker.webp"))
    finally:
        webp_bio.close()

async def tophoto_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.reply_to_message or not update.message.reply_to_message.sticker:
        return await update.message.reply_text("Reply ke sticker untuk dikonversi jadi photo.")
    st = update.message.reply_to_message.sticker
    try:
        f = await context.bot.get_file(st.file_id)
        b = await f.download_as_bytearray()
        img = Image.open(io.BytesIO(b)).convert("RGBA")
    except Exception as e:
        logger.exception("tophoto: failed to get sticker")
        return await update.message.reply_text(f"Gagal ambil sticker: {e}")
    out = io.BytesIO()
    img.convert("RGBA").save(out, format="PNG")
    out.seek(0)
    await context.bot.send_photo(chat_id=update.effective_chat.id, photo=InputFile(out, filename="photo.png"))
    out.close()

async def createpack_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.reply_to_message:
        return await update.message.reply_text("Reply ke photo/sticker dan gunakan: /createpack Title Pack")
    if not context.args:
        return await update.message.reply_text("Format: /createpack Title Pack (contoh: /createpack Funny Memes)")
    title = " ".join(context.args).strip()[:64]
    user = update.effective_user
    raw_name = (user.username or (user.first_name + (user.last_name or ""))).strip()
    if not raw_name:
        raw_name = str(user.id)
    raw_base = re.sub(r'[^a-zA-Z0-9_]', '_', raw_name).lower()
    raw_base = f"{raw_base}_{user.id}"
    bot_user = (await context.bot.get_me()).username
    shortname = make_pack_name_short(raw_base, bot_user)
    reply = update.message.reply_to_message
    try:
        if reply.photo:
            f = await context.bot.get_file(reply.photo[-1].file_id)
            if f.file_size and f.file_size > 5 * 1024 * 1024:
                return await update.message.reply_text("File terlalu besar (>5MB).")
            b = await f.download_as_bytearray()
            img = Image.open(io.BytesIO(b)).convert("RGBA")
        elif reply.sticker:
            f = await context.bot.get_file(reply.sticker.file_id)
            ...
            return await update.message.reply_text("Reply harus photo atau sticker untuk initial sticker.")
    except Exception as e:
        logger.exception("createpack: failed to get file")
        return await update.message.reply_text(f"Gagal ambil file: {e}")
    webp_img = resize_image_for_sticker(img)
    png_bytes = image_to_png_bytes(webp_img)
    user_id = user.id
    try:
        png_bytes.seek(0)
        await context.bot.create_new_sticker_set(
            ...
            name=shortname,
            title=title,
            png_sticker=InputFile(png_bytes, filename="sticker.png"),
            emojis="🙂"
        )
        await update.message.reply_text(f"Sticker pack dibuat: https://t.me/addstickers/{shortname}\n(shortname: {shortname})")
    except Exception as e:
        logger.exception("createpack failed")
        await update.message.reply_text(f"Gagal buat pack: {e}")
    finally:
        png_bytes.close()

async def addtpack_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.reply_to_message:
        return await update.message.reply_text("Reply ke photo/sticker dan gunakan: /addtpack shortname")
    if not context.args:
        return await update.message.reply_text("Masukkan shortname pack. Contoh: /addtpack mypack")
    short_raw = context.args[0]
    bot_user = (await context.bot.get_me()).username
    shortname = make_pack_name_short(short_raw, bot_user)
    reply = update.message.reply_to_message
    try:
        if reply.photo:
            f = await context.bot.get_file(reply.photo[-1].file_id)
            ...
            return await update.message.reply_text("Reply harus photo atau sticker.")
    except Exception as e:
        logger.exception("addtpack: failed to get file")
        return await update.message.reply_text(f"Gagal ambil file: {e}")
    webp_img = resize_image_for_sticker(img)
    png_bytes = image_to_png_bytes(webp_img)
    try:
        png_bytes.seek(0)
        await context.bot.add_sticker_to_set(user_id=update.effective_user.id, name=shortname, png_sticker=InputFile(png_bytes, filename="sticker.png"), emojis="🙂")
        await update.message.reply_text(f"Sticker ditambahkan: https://t.me/addstickers/{shortname}")
    except Exception as e:
        logger.exception("addtpack failed")
        await update.message.reply_text(f"Gagal tambah ke pack: {e}")
    finally:
        png_bytes.close()
